...
def build_BSD_500(model_state_dict, optimizer_state_dict, **kwargs):
    ...
    train_data = dataset.BSD_loader(root=root, split='train',random_crop=True,random_flip=True,normalisation=False)
    ...
    # train_criterion = nn.CrossEntropyLoss(weight=torch.tensor([0.05,0.95])).cuda()
    # eval_criterion = nn.CrossEntropyLoss(weight=torch.tensor([0.05,0.95])).cuda()
...
def main():
    ...
    #     # train_OIS, train_obj = train(train_queue, model, optimizer)
    ...
    if (args.save == True):
        try:
            save_pre_imgs(test_queue, model,ODS=None)
